...
if __name__ == '__main__':
    ref_pose = np.load('pose4.npy')
    test_pose = np.load('pose5.npy')
    ref_pose = ref_pose[VIS_KP]
    test_pose = test_pose[VIS_KP]
    ref_img = cv2.imread("input/test4.jpg")
    test_img = cv2.imread("input/test5.jpg")
    ref_img_with_pose = display_pose(ref_img, ref_pose, color=(0, 0, 255))
    plt.subplot(1, 2, 1)
